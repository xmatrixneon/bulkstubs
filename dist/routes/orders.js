...
async function hasActiveOrder(number, countryid, serviceid) {
...
async function hasRecentUsage(number, countryid, serviceid) {
...
async function isUnderCooldown(number, countryid, serviceid) {
 ...
 */
export async function regetnumber(params) {
    const { api_key, id } = params;
    // Validate API key
    const validation = await validateApiKey(api_key);
    if (!validation.valid) {
